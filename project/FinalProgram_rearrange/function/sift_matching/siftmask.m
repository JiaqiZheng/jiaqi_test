function [ imgfeature ] = siftmask( imgfeature )
...
if size(imgfeature.neighbour,2) == 2 
    ...
    keypoints1 = imgfeature.sift_keypoints(neighbour1_index);
    keypoints2 = imgfeature.sift_keypoints(neighbour2_index);
    ...
    try
        ...
        BW_mask1 = poly2mask(poly_convhull1(:,1),poly_convhull1(:,2),size(imgfeature.image,1),size(imgfeature.image,2));
...
%         imshow(repmat(uint8(BW_mask1),1,1,3).*imgfeature.image)
%         figure;
%         imshow(repmat(uint8(BW_ellipsemask1),1,1,3).*imgfeature.image)
        ...
        BW_mask2 = poly2mask(poly_convhull2(:,1),poly_convhull2(:,2),size(imgfeature.image,1),size(imgfeature.image,2));
        imgfeature.handmask{2} = BW_mask2;
        
        [ BW_ellipsemask2, xy ] = EllipseAroundConvHull( BW_mask2,n, ratio );
%         imshow(repmat(uint8(BW_mask2),1,1,3).*imgfeature.image)
%         figure;
%         imshow(repmat(uint8(BW_ellipsemask2),1,1,3).*imgfeature.image)
...
%         imshow(repmat(uint8(bsxfun(@or,BW_ellipsemask1,BW_ellipsemask2)),1,1,3).*imgfeature.image)
    catch
...
%         imshow(repmat(uint8(BW_mask1),1,1,3).*imgfeature.image)
%         figure;
%         imshow(repmat(uint8(BW_ellipsemask),1,1,3).*imgfeature.image)
