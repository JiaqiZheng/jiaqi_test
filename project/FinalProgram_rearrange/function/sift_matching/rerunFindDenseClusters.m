function [ imgwithsift_structs ] = rerunFindDenseClusters( imgwithsift_structs, threshold_near, twoclusters_index, video_num, show )
...
% this version only contain a single row of imgfeature sturcture
...
for frame_num = 1:1:length(imgwithsift_structs)
    imgfeature1 = imgwithsift_structs{frame_num};
...
%         imgfeature1.sift_descriptors = imgfeature1.sift_descriptors(neighbour_index);
        ...
        if strcmp(show,'dense') || strcmp(show,'all')
            imgshow = cv.drawKeypoints(imgfeature1.image,imgfeature1.sift_keypoints(neighbour_index));
            imshow(imgshow)
            pause(1)
        end
    ...
    imgwithsift_structs{frame_num} = imgfeature1;
