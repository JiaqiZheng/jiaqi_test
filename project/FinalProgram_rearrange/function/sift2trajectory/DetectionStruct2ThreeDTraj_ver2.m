function [ XYZtransformed_struct,imgfeatureRecalculate ] = DetectionStruct2ThreeDTraj_ver2( imgfeature_all,showIdx )
...
% version 2 accept only one row of imgfeature_all (comparing two rows before)
%% 
data_cell = cell(size(showIdx));
imgfeatureRecalculate = cell(size(showIdx));
for idx = 1:1:length(showIdx)
    subNum = showIdx(idx);
    imgfeature2 = imgfeature_all{subNum}(1:end);
