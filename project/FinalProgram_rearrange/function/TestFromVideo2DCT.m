function [ IDCTcell,DCTMatrix,VideoSiftResultNoFace,dissimilarity,data_cell ] = TestFromVideo2DCT( originfolderpath,InputType,CameraAlign,ProcusModel,show)
...
tic
VideoSiftResult = FromRaw2SIFTStruct(originfolderpath,InputType,show);

%% run the kalman filter and interpolation

showIdx = 1:1:length(VideoSiftResult);
[ XYZtransformed_struct,VideoSiftResultAfterKalman ] = DetectionStruct2ThreeDTraj( VideoSiftResult,showIdx );
...
%%
function VideoSiftResult = FromRaw2SIFTStruct(originfolderpath,InputType,show)
...
% videos, this folder contains the video clips.
    ...
    switch InputType
        case 'images'
            ...
            originsign = excludeNonFolder(originsign);
            H = waitbar(0,'Processing...');
            VideoSiftResult = cell(1,length(originsign)); % the VideoSiftResult contains the cells saving all the sift related features 
            for sign_num = 1:1:length(originsign)
                ...
                VideoSiftResult{sign_num} = imgwithsift_origin;
                waitbar(sign_num/length(originsign),H);
            end
            
        case 'video'
            videolist = dir([originfolderpath,'/*.avi']);
            VideoSiftResult = cell(1,length(videolist));

            H = waitbar(0,'Processing...');
            for video_num = 1:1:length(videolist)
                clear video_color video_gray
                k = 1;
                v = VideoReader([originfolderpath,'/',videolist(video_num).name]);
                while hasFrame(v)
                ...
                VideoSiftResult{video_num} = imgwithsift_structs_imitat;
