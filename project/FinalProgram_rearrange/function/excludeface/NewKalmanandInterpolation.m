function [ imgfeature2, centroid_kalman ] = NewKalmanandInterpolation( imgfeature2, sigma_v, phi, outlier_threshold, mode, show )
...
centroid_withoutkalman(outliers_index,:) = NaN;
%% chose only the sign frames interpolation the missing point
% exclude the outlier
[centroid_withoutkalman,HTOutlierIdx] = ExcludeHTOutliersInterpolation(centroid_withoutkalman);
imgfeature2(HTOutlierIdx) = [];
centroid_withoutkalman(HTOutlierIdx,:) = [];
...
if show
    for FrameIdx = 1:1:length(imgfeature2)
        imshow(imgfeature2{FrameIdx}.image);
        hold on
        plot(centroid_kalman(1:FrameIdx,1),centroid_kalman(1:FrameIdx,2),'r-');
        plot(centroid_kalman(FrameIdx,1),centroid_kalman(FrameIdx,2),'g*');
        pause(1)
    end
end
