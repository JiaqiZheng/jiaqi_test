function [ imgfeature2 ] = rerunEllipseRecalculate( imgfeature2, centroid_kalman, show )
%Recalculate the ellipse around the hand
%   Detailed explanation goes here

radius = getMedianFaceSize(imgfeature2);
...
%         radius = (imgfeature2{i_frame}.facebox{:}(3)/2); % instead of using face size for each frame one can use the median of the face bounding box in all the frame  
        skin_mask1 = imgfeature2{i_frame}.SkinMaskNoFace;
...
function radius = getMedianFaceSize(imgfeature2)

for i = 1:1:length(imgfeature2)
    radius = (imgfeature2{i}.facebox{:}(3)/2);
    radiusVect(i) = radius;
end
radius = median(radiusVect);
